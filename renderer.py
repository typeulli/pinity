...
class SpriteRenderer(engine.Component, engine.Drawable):
    ...
    def draw(self) -> None:
        if self.image is None:
            return
        self.gameObject.transform.scene.show(self.image, self.gameObject.transform.position)
...
class Camera(engine.Component, engine.ScreenView):
    def __init__(self, gameObject: engine.GameObject) -> None:
        ...
        self.clearColor: tuple[int, int, int] = (0, 0, 0)
    
        self.view = np.zeros((512, 1024, 3), dtype=np.uint8)
    ...
    def render(self, surface: pygame.Surface, components: list[engine.Component]) -> None:
        if [*surface.get_size()] != [*self.view.shape[:2][::-1]]:
            self.view = np.zeros((surface.get_size()[1], surface.get_size()[0], 3), dtype=np.uint8)
        ...
        surface.blit(pygame.surfarray.make_surface(np.transpose(cv2.cvtColor(self.view, cv2.COLOR_BGR2RGB), (1, 0, 2))), (0, 0)) # type: ignore
    def show(self, image: cv2.typing.MatLike, pos: engine.Vector3) -> None:
        ...
        mask = pos.z > region_z
        region_z[mask] = pos.z
        region_scr[mask] = region_img[mask]
...
if __name__ == "__main__":
    ...
    obj1.addComponent(SpriteRenderer).image = engine.Asset.rectImage(50, 50, (0, 0, 255))
    ...
    renderer2.image = engine.Asset.rectImage(50, 50, (255, 0, 0))


    obj3 = engine.GameObject("obj3", obj2.transform)
    obj3.transform.position = engine.Vector3(12.5, 25, 1)
    obj3.addComponent(SpriteRenderer).image = engine.Asset.rectImage(50, 50, (0, 255, 0))
